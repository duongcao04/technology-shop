...
export interface IFirebaseService {
	...
	getByQuery: <T = unknown>(path: string, childPath: string, value: unknown, limitCount?: number) => Promise<T[]>;
	...
	updateById: <T = unknown>(path: string, id: string, data: Partial<T>) => Promise<void>;
	// Xóa dữ liệu theo ID
...
export interface IClientFirebaseService extends IFirebaseService {
	...
	subscribe: <T = unknown>(path: string, callback: (data: T[]) => void) => () => void;
}
...
export const firebaseService: IFirebaseService = {
	...
	// Lấy dữ liệu theo ID cụ thể
	getById: async <T = unknown>(path: string, id: string): Promise<T | null> => {
		try {
	...
	// Lấy dữ liệu theo điều kiện (filter)
	getByQuery: async <T = unknown>(path: string, childPath: string, value: unknown, limitCount = 10): Promise<T[]> => {
		try {
			const dataRef = ref(database, path);
	...
	// Cập nhật một phần của dữ liệu
	update: async <T = unknown>(path: string, data: Partial<T>): Promise<void> => {
		try {
			const dataRef = ref(database, path);
			// Thêm timestamp
			const dataWithTimestamp = {
	...
	// Cập nhật dữ liệu theo ID
	updateById: async <T = unknown>(path: string, id: string, data: Partial<T>): Promise<void> => {
		try {
			const dataRef = ref(database, `${path}/${id}`);
			// Thêm timestamp
			const dataWithTimestamp = {
